...
function AppShell({ children, onAdd }: { children: ReactNode; onAdd: () => void }) {
  ...
  return (
    <div className="listen-grain min-h-[100dvh] bg-background">
      <aside className="fixed inset-y-0 left-0 z-30 hidden w-[248px] flex-col border-r border-sidebar-border bg-sidebar px-5 py-7 md:flex"><Logo /><p className="mt-14 px-3 font-mono-ui text-[9px] uppercase tracking-[.2em] text-muted-foreground">Навигация</p><nav className="mt-3 space-y-1">{navigation.map(({ href, label, icon: Icon }) => <Link key={href} href={href} className={`flex items-center gap-3 rounded-xl px-3 py-3 text-sm font-semibold transition-colors ${location === href ? 'bg-sidebar-accent text-sidebar-accent-foreground' : 'text-muted-foreground hover:bg-sidebar-accent/60 hover:text-foreground'}`} data-testid={`link-nav-${label}`}><Icon className="size-[17px]" /> {label}</Link>)}</nav><div className="mt-auto rounded-2xl bg-secondary/55 p-4"><p className="font-display text-2xl leading-none">Слушайте<br /><em className="font-normal text-primary">в своём ритме.</em></p><div className="mt-4 flex items-center gap-2 text-[10px] text-muted-foreground"><span className="grid size-6 place-items-center rounded-full bg-primary text-primary-foreground"><Leaf className="size-3" /></span> Без лишнего шума</div></div></aside>
      <header className="sticky top-0 z-20 flex h-[74px] items-center justify-between border-b border-border/70 bg-background/90 px-5 backdrop-blur-md md:hidden"><button onClick={() => setMobileOpen(!mobileOpen)} className="grid size-10 place-items-center rounded-full hover:bg-secondary" aria-label="Открыть меню" data-testid="button-mobile-menu"><Menu className="size-5" /></button><Logo /><button onClick={onAdd} className="grid size-10 place-items-center rounded-full bg-primary text-primary-foreground" aria-label="Добавить книгу" data-testid="button-mobile-add"><Plus className="size-5" /></button></header>
